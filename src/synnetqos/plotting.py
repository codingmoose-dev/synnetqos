...
# Archived distribution-fit plots for optional diagnostic use.
...
def plot_ml_task_result_bars(
    ...
    analysis_role: str | None = "primary",
) -> plt.Figure:
    ...
    if analysis_role is not None and "analysis_role" in plot_df.columns:
        plot_df = plot_df.loc[plot_df["analysis_role"].eq(analysis_role)].copy()
    ...
    if plot_df.empty:
        ...
        ax.text(0.5, 0.5, "No ML benchmark rows available", transform=ax.transAxes, ha="center", va="center")
    ...
    fig_height = max(4.8, 0.34 * len(plot_df))
    ...
    ax.set_xlabel(metric.replace("_", " ").title())
    ax.set_ylabel("")
    if metric in {"average_precision", "roc_auc", "f1", "precision", "recall", "balanced_accuracy"}:
        ax.set_xlim(0, 1.0)
    else:
        ax.set_xlim(left=0)
...
def plot_ml_precision_recall_curves(
    ...
    if not selected:
        ax.text(0.5, 0.5, "No curve data available", transform=ax.transAxes, ha="center", va="center")
    ...
    for row in selected:
        ...
        ax.plot(
            recall,
            precision,
            label=f"{model} (AP={ap:.3f}; positives={positives}/{total})",
        )

        base_rate = float(np.mean(y_true))
        max_precision = max(max_precision, float(np.nanmax(precision)))
    ...
    if base_rate is not None:
        ax.axhline(
            ...
            linestyle="--",
            linewidth=1,
            label=f"Base rate={base_rate:.4f} ({base_rate * 100:.2f}%)",
        )
    ...
    if zoom_to_data and base_rate is not None:
        upper = min(0.12, max(0.02, base_rate * 25.0))
        ax.set_ylim(0, upper)
    else:
        ax.set_ylim(0, 1.02)
